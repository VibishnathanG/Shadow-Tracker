...
export default function TodoFeature() {
  ...
  const [priorityFilter, setPriorityFilter] = useState<TodoFilterPriority>('all');
  const [categoryFilter, setCategoryFilter] = useState<string>('all');
  const [sortBy, setSortBy] = useState<TodoSortOption>('createdAt_desc');
  ...
  return (
    <div className="space-y-6 animate-fadeIn pb-12">
      {/* Top Banner Header */}
      <div className="relative overflow-hidden bg-gradient-to-r from-emerald-950/70 via-teal-950/60 to-surface-elevated border border-emerald-500/30 rounded-3xl p-4 sm:p-5 shadow-xl">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 relative z-10">
          <div className="space-y-1">
            <div className="flex items-center gap-2 sm:gap-2.5">
              <span className="p-1.5 bg-emerald-500/20 text-emerald-400 rounded-xl border border-emerald-500/30 shadow-sm shrink-0">
              ...
              <div className="flex flex-wrap items-center gap-2 min-w-0">
                ...
                <span className="inline-flex items-center gap-1 px-2 py-0.5 text-[8.5px] sm:text-[9.5px] font-black uppercase tracking-wider bg-emerald-500/15 text-emerald-300 border border-emerald-500/30 rounded-full shrink-0 shadow-2xs">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 sm:gap-3 w-full lg:w-auto">
            <div className="grid grid-cols-4 gap-1.5 sm:gap-2 w-full sm:w-auto">
              <div className="bg-surface/70 backdrop-blur-md border border-border/60 rounded-xl px-2 py-1.5 sm:px-3 sm:py-2 text-center min-w-0">
              ...
              <div className="bg-surface/70 backdrop-blur-md border border-border/60 rounded-xl px-2 py-1.5 sm:px-3 sm:py-2 text-center min-w-0">
                <span className="text-[9px] sm:text-[10px] font-bold uppercase tracking-wider text-amber-400 block truncate">Active</span>
                <span className="text-sm sm:text-base font-black text-amber-400">{activeCount}</span>
              </div>
              <div className="bg-surface/70 backdrop-blur-md border border-border/60 rounded-xl px-2 py-1.5 sm:px-3 sm:py-2 text-center min-w-0">
                <span className="text-[9px] sm:text-[10px] font-bold uppercase tracking-wider text-emerald-400 block truncate">Done</span>
                <span className="text-sm sm:text-base font-black text-emerald-400">{completedCount}</span>
              </div>
              <div className="bg-surface/70 backdrop-blur-md border border-border/60 rounded-xl px-2 py-1.5 sm:px-3 sm:py-2 text-center min-w-0">
                <span className="text-[9px] sm:text-[10px] font-bold uppercase tracking-wider text-cyan-400 block truncate">Star</span>
                <span className="text-sm sm:text-base font-black text-cyan-400">{starredCount}</span>
              </div>
            </div>

            <button
              onClick={() => setShowGraph(prev => !prev)}
              className="flex items-center justify-center gap-1.5 px-3 py-2 bg-surface-elevated hover:bg-secondary text-muted-foreground hover:text-foreground text-xs font-bold rounded-xl border border-border transition-all cursor-pointer w-full sm:w-auto shrink-0"
              title="Toggle 15-Day Graph"
            >
              <Lucide.TrendingUp size={14} className="text-emerald-400" />
        ...
        {totalCount > 0 && (
          <div className="mt-3.5 pt-3 border-t border-emerald-500/20 relative z-10">
            <div className="flex items-center justify-between text-[11px] font-bold mb-1.5">
              <span className="text-muted-foreground uppercase tracking-wider">Overall Progress</span>
              <span className="text-emerald-400">{completionPercent}% ({completedCount}/{totalCount})</span>
            </div>
            <div className="w-full h-2 bg-surface/80 rounded-full overflow-hidden border border-emerald-500/20 p-0.5">
              <motion.div
                initial={{ width: 0 }}
        ...
        <AnimatePresence>
          {showGraph && (
            <motion.div
              ...
              className="mt-3.5 pt-3 border-t border-emerald-500/20 relative z-10 space-y-2 overflow-hidden"
            >
              <div className="flex items-center justify-between gap-2">
                <h4 className="text-[11px] font-extrabold uppercase tracking-wider text-white flex items-center gap-1.5">
                  <Lucide.TrendingUp size={13} className="text-emerald-400" />
              ...
              <div className="bg-surface/50 backdrop-blur-md border border-border/40 rounded-xl px-3 py-2">
                <div className="gap-1 sm:gap-1.5 h-16 items-end pt-2 pb-0.5" style={{ display: 'grid', gridTemplateColumns: 'repeat(15, minmax(0, 1fr))' }}>
                  {last15DaysGraph.days.map((item, idx) => {
                    ...
                    return (
                      ...
                      >
                        {/* Hover Tooltip */}
                        <div className="absolute -top-10 left-1/2 -translate-x-1/2 bg-surface-elevated border border-emerald-500/40 text-foreground px-2 py-0.5 rounded-lg text-[9px] font-bold whitespace-nowrap shadow-xl opacity-0 group-hover/bar:opacity-100 transition-all pointer-events-none z-20">
                          <span className="text-emerald-400">{item.label}</span>: {item.committed} commit, {item.completed} done
                        </div>
                        ...
                        <div className="w-full flex items-end justify-center gap-0.5 h-full">
                          ...
                          <div
                            ...
                            style={{ height: `${item.committed > 0 ? Math.max(15, committedPct) : 4}%` }}
                          />
                          {/* Completed Bar */}
                          <div
                            className={`w-1 sm:w-2 rounded-t-xs transition-all duration-300 ${
                            ...
                            style={{ height: `${item.completed > 0 ? Math.max(15, completedPct) : 4}%` }}
                          />
                        </div>

                        {/* X-Axis Date Label */}
                        <span className={`text-[8px] font-bold tracking-tighter truncate mt-1 ${isToday ? 'text-emerald-400 font-black' : 'text-muted-foreground'}`}>
      ...
      <div className="bg-surface border border-border/80 rounded-2xl p-4 shadow-lg space-y-3">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          {/* Status Tabs */}
          <div className="flex items-center gap-1 p-1 bg-surface-elevated rounded-xl border border-border/60 overflow-x-auto scrollbar-none">
            {(['all', 'active', 'completed', 'starred'] as TodoFilterStatus[]).map(tab => {
              ...
              return (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-bold tracking-wider capitalize transition-all shrink-0 cursor-pointer ${
                    isActive
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <div className="flex items-center p-1 bg-surface-elevated rounded-xl border border-border/60">
              <button
            ...
            <motion.button
              ...
              className="flex items-center gap-1.5 px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs uppercase tracking-wider rounded-xl shadow-md shadow-emerald-600/30 transition-all cursor-pointer"
            >
              <Lucide.Plus size={15} />
              <span>Add ToDo</span>
            </motion.button>
      ...
      {filteredTodos.length === 0 ? (
        ...
        <div className={viewMode === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3.5' : 'space-y-3.5'}>
